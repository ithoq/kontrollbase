...
NULL , '$system_user_id', '$page_id', '$host_id', NOW( )
  ...
  function get_overview_stats() {
    ...
    if($user_server_client_id == 0) { //user is not a client so we load all hosts
      ...
      //we grab our data from the analytics_overview_data table (which is updated daily via an "INSERT INTO ... SELECT" query that reads data from the view_analytics_overview table) - we do NOT use the commented out query (which is actually used to create the view table data) because it is rather expensive and makes login take too long on a schema that has 1) lots of hosts, 2) long date range of data, 3) both 1 and 2
      ...
      $sql = "select * from analytics_overview_data order by id desc limit 1";
    }
  ...
  function get_host_growth_30($server_list_id) {
    ...
    $dbr = $this->load->database('read', TRUE);
    $sql = "select (select (((length_data + length_index) / 1024) / 1024) as curr_size from server_statistics where server_list_id='$server_list_id' and CURDATE() <= Creation_time order by Creation_time asc limit 1) as 0_day_size_mb, 

(select (((length_data + length_index) / 1024) / 1024) as curr_size from server_statistics where server_list_id='$server_list_id' and DATE_SUB(CURDATE(),INTERVAL 30 DAY) <= Creation_time order by Creation_time asc limit 1) as 30_day_size_mb,

 ( (select (((length_data + length_index) / 1024) / 1024) as curr_size from server_statistics where server_list_id='$server_list_id' and CURDATE() <= Creation_time order by Creation_time asc limit 1) - (select (((length_data + length_index) / 1024) / 1024) as curr_size from server_statistics where server_list_id='$server_list_id' and DATE_SUB(CURDATE(),INTERVAL 30 DAY) <= Creation_time order by Creation_time asc limit 1)) as difference";
    log_message('debug', "$sql");
    qstart();
    $query = $dbr->query($sql);
    qend();
...
canvasBorderColor='b8b8b8' baseFontColor='666666' lineColor='99ccff' lineThickness='1'>\n";
  ...
  function get_graphs_sec($xval,$yaxis,$name,$server_list_id,$sday,$eday) {
    ...
    if($server_list_id== '0') {
      ...
      //The following query does not use $sday and $eday, runs a bit quicker on large datasets but cannot be used with the (new) main page cumulative graphs page. 
