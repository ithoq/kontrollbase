...
<h1>Kontrollbase Client setup process on Ubuntu / Debian Servers</h1>
    These instructions apply to the following Linux flavors: Ubuntu, KUbuntu, Debian. In fact, most Linux'es that use apt-get will see this document as useful. This has been tested on Ubuntu 8.10<br><br>
    <p class="important">
...
required module. Repeat as needed for the other modules. If they are not available on apt-get you can get them on CPAN.
<br><br>
...
<p class="important">
    <strong>Note</strong>: You need to create the "kontrollbase" user on <strong>every client server</strong> that Kontrollbase will be monitoring.
The account needs the following permissions: SELECT,PROCESS on every single schema that will be monitored.
</p>
...
<h2>2. Install SNMP and SNMPd using apt-get</h2>
...
<h2>3. Modify the config file</h2>
...
<h2>4. Create Kontrollbase system user on client server and set password</h2>
...
<h2>5. Login remotely, create ssh-key and then copy public key to authorized_keys file</h2>
...
<h2>6. Login again to make sure ssh key is working correctly.</h2>
...
<h2>7. copy the kontrollbase client script to the host</h2>
...
<h2>8. Install perl modules if needed</h2>
...
<h2>9. Test the client script without flags</h2>
